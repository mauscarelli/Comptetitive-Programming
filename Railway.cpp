...
int main()
{
	...
	while(cin >> S){
        ...
        for(int i = 0;i<vet.size()-1;i++){
            ...
            t = distPontoSeg(vet[i],vet[i+1],S);
            if(t < M){
                M = t;
                if(cross(S-vet[i],vet[i+1]-vet[i]) == 0){ 
                    //out << "oi ";
                    if(dist(S,vet[i]) < dist(S,vet[i+1])) mini = vet[i];
                    else mini = vet[i+1];
                    //cout << mini << endl;
                }
                else mini = vet[i] + proj(S-vet[i],vet[i+1]-vet[i]);
            }
        }
        if(dist(vet[vet.size()-1],S) < M)
                mini = vet[vet.size()-1];
        printf("%.4f\n%.4f\n",mini.x,mini.y);
    }
}
